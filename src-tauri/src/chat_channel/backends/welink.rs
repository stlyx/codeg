...
impl WelinkBackend {
    ...
    async fn send_html(&self, html: &str) -> Result<SentMessageId, ChatChannelError> {
        ...
        for chunk in chunks {
            ...
            eprintln!(
                "[WeLink] sent chunk channel={} chars={} response={} text={}",
                self.channel_id,
                chunk.chars().count(),
                preview_text(&text, 160),
                preview_text(&html_to_text(&chunk), 160)
            );
...
impl ChatChannelBackend for WelinkBackend {
    ...
    async fn start(
        ...
        *self.status.lock().await = ChannelConnectionStatus::Connecting;
        ...
        eprintln!(
            "[WeLink] channel={} started group={} cursor={:?} include_sender={:?} exclude_sender={:?} initial_stats={:?}",
            self.channel_id,
            self.group_id,
            cursor,
            self.include_sender,
                ...
                match output {
                    Ok(stdout) => {
                        match parse_history_output(
                            ...
                            Ok(batch) => {
                                if batch.stats.has_activity() {
                                    eprintln!(
                                        "[WeLink] channel={} poll cursor={:?}->{:?} stats={:?}",
                                        channel_id, cursor, batch.next_cursor, batch.stats
                                    );
                                }
                                ...
                                *status.lock().await = ChannelConnectionStatus::Connected;
                                for msg in batch.messages {
                                    eprintln!(
                                        "[WeLink] dispatch channel={} msg_id={:?} sender={} content_type={:?} text={}",
                                        channel_id,
                                        message_id_for_log(&msg.metadata),
                                        msg.sender,
                                        msg.metadata
                                            .get("contentType")
                                            .and_then(serde_json::Value::as_str),
                                        preview_text(&msg.text, 160)
                                    );
...
async fn run_cli_command(
    ...
    let mut command = crate::process::tokio_command(&words[0]);
...
struct ParsedHistoryBatch {
    ...
    stats: HistoryParseStats,
}

#[derive(Debug, Default, PartialEq)]
struct HistoryParseStats {
    total: usize,
    old_or_cursor: usize,
    filtered_sender: usize,
    empty_text: usize,
    accepted: usize,
    senders: Vec<String>,
}

impl HistoryParseStats {
    fn has_activity(&self) -> bool {
        self.total > 0 || self.accepted > 0 || self.filtered_sender > 0 || self.empty_text > 0
    }
}
...
fn parse_history_output(
    ...
    if initial_sync {
        return Ok(ParsedHistoryBatch {
            messages: Vec::new(),
            next_cursor,
            stats: history_stats(&messages, previous_cursor, include_sender, exclude_sender),
        });
    }

    let lower_bound = previous_cursor.unwrap_or(0);
    let stats = history_stats(&messages, previous_cursor, include_sender, exclude_sender);
    let parsed = messages
        .into_iter()
...
fn history_stats(
    ...
    let lower_bound = previous_cursor.unwrap_or(0);
    let mut stats = HistoryParseStats {
        total: messages.len(),
        ..HistoryParseStats::default()
    };

    for message in messages {
        let sender = message_sender(message);
        if !stats.senders.iter().any(|existing| existing == &sender) {
            stats.senders.push(sender.clone());
        }

        if json_u64(message, "msgId").is_some_and(|id| id <= lower_bound) {
            stats.old_or_cursor += 1;
            continue;
        }

        if !sender_allowed(&sender, include_sender, exclude_sender) {
            stats.filtered_sender += 1;
            continue;
        }

        match extract_message_text(message).map(|text| text.trim().to_string()) {
            Some(text) if !text.is_empty() => stats.accepted += 1,
            _ => stats.empty_text += 1,
        }
    }
...
fn message_sender(message: &serde_json::Value) -> String {
    message
        .get("sender")
        .and_then(value_to_string)
        .map(|sender| sender.trim().to_string())
        .filter(|sender| !sender.is_empty())
        .unwrap_or_else(|| "welink-user".to_string())
}
...
fn message_id_for_log(message: &serde_json::Value) -> Option<String> {
    message.get("msgId").and_then(value_to_string)
}
...
mod tests {
    ...
    #[test]
    fn exclude_sender_filters_welink_bot_echo() {
        let output = sample_history(
            vec![
                serde_json::json!({
                    "msgId": 6,
                    "sender": "p_xiaoluban",
                    "contentType": "TEXT_MSG",
                    "content": "OpenCode 正在响应中..."
                }),
                serde_json::json!({
                    "msgId": 7,
                    "sender": "stlyx",
                    "contentType": "TEXT_MSG",
                    "content": "/task 写一个 markdown 示例"
                }),
            ],
            7,
        );
        let exclude = vec!["p_xiaoluban".to_string()];
        let batch = parse_history_output(&output, Some("5"), &[], &exclude, false).expect("parse");

        assert_eq!(batch.messages.len(), 1);
        assert_eq!(batch.messages[0].sender, "stlyx");
        assert_eq!(batch.messages[0].text, "/task 写一个 markdown 示例");
    }

    #[test]
    fn sender_filter_accepts_numeric_sender_values() {
        let output = sample_history(
            vec![serde_json::json!({
                "msgId": 6,
                "sender": 42,
        ...
        let include = vec!["42".to_string()];
        let batch = parse_history_output(&output, Some("5"), &include, &[], false).expect("parse");

        assert_eq!(batch.messages.len(), 1);
        assert_eq!(batch.messages[0].sender, "42");
    }
